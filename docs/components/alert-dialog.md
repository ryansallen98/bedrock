...
- **`content` `duration`:** Tailwind duration segment (e.g. **`duration-100`**); leave empty to use **`components.alert_dialog.meta.default_duration_segment`**. Applied to overlay/panel strings and **`primitive.portal`** **`x-transition`** enter/leave.
